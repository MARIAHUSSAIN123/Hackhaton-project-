...
.stApp {
    background:
        radial-gradient(
            circle at 10% -8%,
            rgba(244, 162, 97, 0.16) 0%,
            transparent 38%
        ),
        radial-gradient(
            circle at 92% 8%,
            rgba(47, 191, 159, 0.13) 0%,
            transparent 42%
        ),
        radial-gradient(
            circle at 30% 105%,
...
@keyframes glowPulse {
    ...
    100% {
        box-shadow:
            0 0 12px rgba(244, 162, 97, 0.18);
    }

    50% {
        box-shadow:
            0 0 26px rgba(244, 162, 97, 0.4);
    }
...
.kpi:hover {
    ...
    box-shadow:
        ...
        0 0 0 1px var(--accent, var(--teal)) inset;
}
...
@st.cache_data(
    show_spinner="Loading and cleaning delivery records..."
)
def get_data():
    ...
    overview = A.data_overview(
        raw
    )

    clean, log = A.clean_data(
        raw
    )
...
with st.sidebar:
    ...
    st.caption(
        "Reads the matching API key from environment "
...
if len(f) == 0:
    ...
    st.warning(
        "No rows match these filters — widen your selection."
    )
...
with tab_overview:
    ...
    with c2:
        ...
        st.markdown(
            ...
            - **Duplicates dropped:**
              {clean_log['duplicates_dropped']}

            - **Age** missing
              ({clean_log['age_missing_filled']} rows)
              → filled with median =
              **{clean_log['age_fill_value']}**

            - **Rating** missing
              ({clean_log['rating_missing_filled']} rows)
              → filled with median =
              **{clean_log['rating_fill_value']}**

            - **Order time** stored in two formats in the raw file
              (clock strings and Excel fraction-of-day floats) —
...
with tab_q2:
    ...
    with c2:
        ...
        fig3.update_layout(
            ...
            title=
                "Average time by distance bucket",
...
with tab_q3:
    ...
    fig4.update_layout(
        ...
        title=
            "Average delivery time (min): weather × traffic",
...
with tab_chat:
    ...
    st.markdown(
        ...
        <div class="chat-info">
            ...
            <b>currently filtered dashboard data</b>.

            Change the filters from the sidebar and ask again
    ...
    with status_col:
        ...
        st.caption(
            ...
            f"📦 Analyzing "
            f"{len(f):,} currently filtered deliveries"
